...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  void initState() {
    ...
    // Önce profil verilerini yükle ve hesapla (su hedefi için gerekli)
    _loadProfileAndCalculate().then((_) {
      // Profil yüklendikten sonra su sayısını getir (hedef doğru olacak)
...
Her öneri çok kısa olsun: maksimum 1 cümle ve 10-15 kelime. Önerileri numaralandırma (1., 2., 3. gibi) veya madde işareti (-) kullanmadan, sadece metin olarak ver. Her öneri ayrı bir satırda olsun. Türkçe yanıt ver. Sadece önerileri yaz, başlık veya açıklama ekleme.''';
  ...
  // SADECE öğünler (daily_meals) tablosundan, sadece fotoğrafı olan öğünlerin kalorilerini sayar
  Future<int> _getTodayCalories() async {
    try {
      ...
      // SADECE daily_meals tablosundan bugünün kalorilerini çek
      // Sadece image_url null olmayan kayıtların kalorilerini say (fotoğrafı olan öğünler)
      final dailyMealsResult = await _supabaseHelper.executeQuerySilent(
        () => supabase
            .from('daily_meals')
            .select('calorie, image_url')
            .eq('user_id', _userId!)
            .eq('date', _today),
      );
      ...
      if (dailyMealsResult != null) {
        ...
        for (var item in meals) {
          final mealMap = item as Map<String, dynamic>?;
          if (mealMap == null) continue;
          
          final imageUrl = mealMap['image_url'] as String?;
          // Sadece fotoğrafı olan öğünlerin kalorilerini say (image_url null değil ve boş değil)
          if (imageUrl != null && imageUrl.isNotEmpty) {
            final calorie = mealMap['calorie'] as num?;
